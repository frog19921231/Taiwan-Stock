    ...
    try:
        ...
        if data.get("msg") == "success" and len(data.get("data", [])) > 0:
            ...
            for record in latest_records:
                ...
                if "外資" in name:
                    chip_result["外資"] += volume_in_lots
                elif "投信" in name:
                    chip_result["投信"] += volume_in_lots
                elif "自營商" in name:
                    chip_result["自營商"] += volume_in_lots
    except Exception:
...
def get_marquee_html():
    hot_list = ["2330", "2317", "3231", "2603", "2454", "2382", "3481", "2618", "2356", "2891"]
    params = "|".join([f"tse_{sid}.tw" for sid in hot_list])
    ...
    try:
        ...
        for s in top_stocks:
            if s['pct'] > 0:
                ...
                color, arrow = "white", "-"
            html_content += f"<a href='/?target_stock={s['id']}' target='_self' style='text-decoration:none; color:{color}; margin-right: 40px; font-size: 18px; font-weight: bold;' title='點擊將 {s['name']} 加入分析'>{s['name']} {s['price']} {arrow} {s['pct']:.2f}%</a>"
        return f"""
        ...
        return "<div style='color: gray;'>即時行情跑馬燈載入中...</div>"
...
with col_left:
    ...
    st.caption(f"聲量最後更新: {st.session_state.ptt_last_update_str} (每 1 小時自動刷新)")
    ...
    if st.button("🗑️ 清空監控清單與警報紀錄"):
        ...
        st.session_state.alert_history.clear()  # 🌟 清空警報歷史
        st.rerun()
    ...
    if st.session_state.monitored_stocks:
        ...
        with tab2:
            with st.spinner("向 FinMind 請求最新真實籌碼中..."):
                ...
                st.caption(f"📅 資料日期: **{latest_date}** (盤後公布)")
                chip_cols = st.columns(3)
                for idx, (inst, value) in enumerate(chip_data.items()):
                    with chip_cols[idx]:
                        st.metric(label=inst, value=f"{value} 張", delta="買超" if value > 0 else "賣超",
                                  delta_color="normal")
